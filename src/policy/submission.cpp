 ...
 * @return Move 
 */
int absub(State *now, int depth, bool maximizingPlayer, int alpha, int beta){
    ...
    if(maximizingPlayer){
        ...
        for(auto child : actions){
            ...
            int temp = absub(childState, depth-1, 0, alpha, beta);
            if(temp > alpha){
                alpha = temp;
            }
            if(alpha >= beta){
        ...
        for(auto child : actions){
            State *childState = now->next_state(child);
            int temp = absub(childState, depth-1, 1, alpha, beta);
            if(temp < beta){
                beta = temp;
            }
            if(alpha >= beta){
...
Move abs::get_move(State *state, int depth){
